...
class angel_broking_execution():
 ...
 def get_creds_for_broker(self):    
     broker_creds=self.fs.fetch_user_creds(self.chat_id)
     return broker_creds

 def generate_client(self):
  try:
   print("generating the client with existing token")
   broker_creds=self.get_creds_for_broker()
   obj=SmartConnect(api_key=broker_creds['api_key'])
   
   self.session = obj.generateSession(broker_creds['client_id'],broker_creds['password'])
 ...
 def place_order(self,transaction_type_,order_type_,scrip,token,price,sl,qty):
  ...
  print("Placing Equity order for angel broking") 
  print(orderparams)    
  order_id=  self.broker_object.placeOrder(orderparams)
  print(order_id)
  time.sleep(0.1)
  #print("order book",self.broker_object.orderBook())
  ...
  for order in order_book:
     ...
     if order['orderid'] == order_id:             
             ...
             print("status loop",order_status)
             break
     
  order_start_time = time.time()
  print("status",order_status)
   
  if order_status not in ('rejected'):  
